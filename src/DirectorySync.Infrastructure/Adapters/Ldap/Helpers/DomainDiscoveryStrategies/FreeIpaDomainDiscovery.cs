...
internal sealed class FreeIpaDomainDiscovery : IDomainDiscoveryStrategy
{
    ...
    {
        var entries = connection.QueryDomains(
            $"cn=ipaconfig,cn=etc,{schema.NamingContext.StringRepresentation}",
        ...
        {
            var value = entry.GetAttributeValue("ipaDomainName");
    ...
    {
        var entries = connection.QueryDomains($"cn=trusts,cn=etc,{schema.NamingContext.StringRepresentation}",
        ...
        foreach (SearchResultEntry entry in entries)
        {
            var value = entry.GetAttributeValue("ipaNTTrustPartner");
            if (!string.IsNullOrWhiteSpace(value))
            {
